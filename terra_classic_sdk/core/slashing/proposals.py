"""Slashing module governance proposal types."""

from __future__ import annotations

import attr
from betterproto.lib.google.protobuf import Any as Any_pb
from terra_proto.cosmos.slashing.v1beta1 import (
    MsgUpdateParams as MsgUpdateParams_pb
)

from terra_classic_sdk.core import AccAddress
from terra_classic_sdk.util.json import JSONSerializable
from terra_classic_sdk.core.numeric import Numeric, Dec

__all__ = ["MsgUpdateParams"]


@attr.s
class Params(JSONSerializable):
    """Slashing module parameters."""

    signed_blocks_window: Numeric = attr.ib()
    min_signed_per_window: Dec = attr.ib()
    downtime_jail_duration: str = attr.ib()
    slash_fraction_double_sign: Dec = attr.ib()
    slash_fraction_downtime: Dec = attr.ib()

    @classmethod
    def from_data(cls, data: dict) -> Params:
        return cls(
            signed_blocks_window=Numeric.parse(data["signed_blocks_window"]),
            min_signed_per_window=Dec(data["min_signed_per_window"]),
            downtime_jail_duration=data["downtime_jail_duration"],
            slash_fraction_double_sign=Dec(data["slash_fraction_double_sign"]),
            slash_fraction_downtime=Dec(data["slash_fraction_downtime"]),
        )

    def to_data(self) -> dict:
        return {
            "signed_blocks_window": str(self.signed_blocks_window),
            "min_signed_per_window": str(self.min_signed_per_window),
            "downtime_jail_duration": self.downtime_jail_duration,
            "slash_fraction_double_sign": str(self.slash_fraction_double_sign),
            "slash_fraction_downtime": str(self.slash_fraction_downtime),
        }


@attr.s
class MsgUpdateParams(JSONSerializable):
    """Proposal for slashing params updating.

    Args:
        authority: the address that controls the module
        params: defines the x/slashing parameters to update
    """

    type_amino = "slashing/MsgUpdateParams"
    """"""
    type_url = "/cosmos.slashing.v1beta1.MsgUpdateParams"
    """"""
    prototype = MsgUpdateParams_pb
    """"""
    authority: AccAddress = attr.ib()
    """
    authority is the address that controls the module (defaults to x/gov unless
    overwritten).
    """
    params: Params = attr.ib()
    """
    params defines the x/slashing parameters to update. NOTE: All
    parameters must be supplied.
    """

    def to_amino(self) -> dict:
        return {
            "type": self.type_amino,
            "value": {
                "authority": self.authority,
                "params": self.params.to_data(),
            },
        }

    @classmethod
    def from_data(cls, data: dict) -> MsgUpdateParams:
        return cls(
            authority=data["authority"],
            params=Params.from_data(data["params"]),
        )

    def to_data(self) -> dict:
        return {
            "@type": self.type_url,
            "authority": self.authority,
            "params": self.params.to_data(),
        }

    def to_proto(self) -> MsgUpdateParams_pb:
        return MsgUpdateParams_pb(
            authority=self.authority,
            params=self.params.to_proto(),
        )

    @classmethod
    def from_proto(cls, proto: MsgUpdateParams_pb) -> MsgUpdateParams:
        return cls(
            authority=proto.authority,
            params=Params.from_proto(proto.params),
        )

    def pack_any(self) -> Any_pb:
        return Any_pb(type_url=self.type_url, value=bytes(self.to_proto()))
